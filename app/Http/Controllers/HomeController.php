...
class HomeController extends Controller
{
    ...
    public function cart()
    {
        ...
        curl_setopt_array($curl, array(
            ...
            CURLOPT_HTTPHEADER => array(
                "key: 771972d748c3f345c593278340555e5b"
            ),
        ));
    ...
    public function get_ongkir($destination, $weight)
    {
        ...
        curl_setopt_array($curl, array(
            ...
            CURLOPT_POSTFIELDS => "origin=255&destination=" . $destination . "&weight=" . $weight . "&courier=jne",
            CURLOPT_HTTPHEADER => array(
    ...
    public function proses_input_testimoni(Request $request)
    {
        // Validasi data yang dikirim dari form
        $validatedData = $request->validate([
            'nama_testimoni' => 'required|max:255',
            'deskripsi' => 'required',
            'gambar' => 'image|mimes:jpeg,png,jpg,gif|max:2048', // Contoh validasi untuk file gambar (opsional)
        ]);

        // Simpan data testimoni ke dalam database menggunakan model Testimoni
        $testimoni = new Testimoni();
        $testimoni->nama_testimoni = $request->nama_testimoni;
        $testimoni->deskripsi = $request->deskripsi;

        // Jika ada file gambar yang diunggah, simpan juga file gambar
        if ($request->hasFile('gambar')) {
            $gambar = $request->file('gambar');
            $nama_file = time() . '_' . $gambar->getClientOriginalName();
            $gambar->move(public_path('images'), $nama_file);
            $testimoni->gambar = $nama_file;
        }

        $testimoni->save();

        // Redirect ke halaman lain atau lakukan yang diperlukan setelah menyimpan testimoni
