                        ...
                        <div class="col-md-7">
                            <div class="actions">
                                <div class="wc-proceed-to-checkout">
                                    <a href="#" class="btn btn-lg btn-dark checkout"><span>
                                            checkout</span></a>
                                </div>
                            </div>
                        </div>
                        ...
                        <table class="table shop_table">
                            <tbody>
                                <tr class="cart-subtotal">
                                    <th>Cart Subtotal</th>
                                    <td>
                                        <span class="amount cart-total">Rp. {{$cart_total}}</span>
                                    </td>
                                </tr>
                                <tr class="shipping">
                                    <th>Shipping</th>
            ...
            $('.update-total').click(function(e){
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        // Dapatkan nilai numerik dari string
                        const biayaPengiriman = parseInt(data.rajaongkir.results[0].costs[0].cost[0].value);
                        const totalKeranjang = parseInt($('.cart-total').text().replace('Rp. ', '').replace(',', ''));
            
                        // Periksa jika nilai numerik valid
                        if (!isNaN(biayaPengiriman) && !isNaN(totalKeranjang)) {
                            const grandTotal = biayaPengiriman + totalKeranjang;
            
                            // Tampilkan nilai total dan biaya pengiriman dengan format "Rp."
                            $('.shipping-cost').text('Rp. ' + biayaPengiriman.toLocaleString('id-ID'));
                            $('.grand-total').text('Rp. ' + grandTotal.toLocaleString('id-ID'));
                            $('.grand_total').val(grandTotal);
                        } else {
                            // Tampilkan pesan kesalahan jika terdapat nilai yang tidak valid
                            console.error('Ada kesalahan dalam perhitungan.');
                        }
                    }
                });
            });
